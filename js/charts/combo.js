...
function combo() {
    ...
    var _localSVG,
        ...
        _localMin = 0,
        _localMax = 0,
        _localMeasureBarChart = [],
        _localMeasureAreaChart = [],
        _localLegend,
        _localTooltip;
        // _localLabelStack;
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            _measure.forEach(function(m, i) {
                if(_measureChartType[i] == 'bar') {
                    _localMeasureBarChart.push(m);
                } else if(_measureChartType[i] == 'area') {
                    _localMeasureAreaChart.push(m);
                }
            });
            ...
            _xMeasure.domain(_localMeasureBarChart)
            ...
            /* Bar Plot */
            _drawBarPlot.apply(plot, [plotHeight]);

            /* Line Plot */
            _drawLinePlot.apply(plot, [plotHeight]);
    ...
    chart.update = function(data) {
        ...
        /* Update Bar */
        var clusterGroupBar = svg.select('#cluster-group-bar').selectAll('.cluster')
            .data(data);
        ...
        lineGroup.select('path.area-path')
            .datum(function(m, i) {
                var clone = jQuery.extend(true, [], _localData); // deep copy
        ...
        var linePoint = lineGroup.selectAll('.line-point')
            .data(function(m, i) {
                var clone = jQuery.extend(true, [], _localData); // deep copy
