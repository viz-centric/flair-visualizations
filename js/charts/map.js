...
function map() {
    ...
    var _local_svg, _Local_data, _originalData, parentContainer, valueMapper = [], container, path, projection;
    ...
    function ready(mapData, countries) {
        ...
        var round = container.selectAll('circle').data(Topojson.feature(mapData, mapData.objects.countries).features);

        round.enter().append('circle')
            .attr('cx', function (d) {
        ...
        var labels = container.selectAll('text.label').data(Topojson.feature(mapData, mapData.objects.countries).features);
        ...
        labels.enter().append('text')
            ...
            .text(function (d) {
                if (valueMapper[d.properties.name]) {
                    return UTIL.getFormattedValue(valueMapper[d.properties.name], UTIL.getNumberFormatterFn('M', valueMapper[d.properties.name]));
                }
                else {
                    return '';
                }
