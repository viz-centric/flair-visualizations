...
function multilevelgrouping() {
    ...
    var _Local_data, _showNavigation = true,
        filterData = [],
        _originalData, _local_svg, parentContainer, activePage = 0;
    ...
    var createTable = function (data) {
        ...
        var table = _local_svg.append('table')
            .attr('id', 'viz_multilevelgrouping')
            //.style('width', '100%')
        ...
        if (!_print) {
            ...
            var rowGroup = [],
                ...
                orderData = [];
            ;
            for (let index = 0; index < _dimension.length - 1; index++) {
                rowGroup.push(index);
                orderData.push([index])
            }
            
            for (let index = _dimension.length - 2; index >= 0; index--) {
                columnDefs.push(index);
            }
            dataTable = $('#' + parentContainer.attr('id')).find('#viz_multilevelgrouping').DataTable({
                "orderCellsTop": true,
                "order": orderData,
                "rowGroup": {
                    dataSrc: rowGroup
            ...
            $("#viz_multilevelgrouping_paginate").css('display', 'blobk')
...
module.exports = multilevelgrouping;